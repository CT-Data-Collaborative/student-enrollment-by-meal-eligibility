...
##################################################################
#
# Processing Script for Student-Enrollment-by-Meal-Eligibility
...
cols_to_stack <- c("Free", "Non-Subsidized", "Reduced")
...
complete_student_enrollment_long <- reshape(complete_student_enrollment,
                                            ...
                                            v.names = "Value",
                                            timevar = "Free/Reduced Price Meal Eligibility",
                                            times = cols_to_stack,
...
complete_student_enrollment_long$`Free/Reduced Price Meal Eligibility`[complete_student_enrollment_long$`Free/Reduced Price Meal Eligibility` == "Free"] <- "Eligible For Free Lunch"
complete_student_enrollment_long$`Free/Reduced Price Meal Eligibility`[complete_student_enrollment_long$`Free/Reduced Price Meal Eligibility` == "Non-Subsidized"] <- "Not Eligible For Lunch"
complete_student_enrollment_long$`Free/Reduced Price Meal Eligibility`[complete_student_enrollment_long$`Free/Reduced Price Meal Eligibility` == "Reduced"] <- "Eligible For Reduced Lunch"
...
test <- complete_student_enrollment_long[,c("District", "Year", "Free/Reduced Price Meal Eligibility")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_student_enrollment_long,
  file.path(path_to_top_level, "data", "student_enrollment_by_meal_eligibility_2008-2021.csv"),
  sep = ",",
  row.names = F
)